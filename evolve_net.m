...
%                 old_x = x_end(i);
                ...
                if mod(iterations, 10)==0
                    subplot(1,2,1);
                    vis(x_end);
                    title(sprintf('%d Iterations', iterations),...
                         'Fontsize', 16);
                    subplot(1,2,2);
                    hold on;
                ...
                h = sprintf('Done in: %d iterations', iterations);
                ...
                if iterations>1
                    ...
                    if ~isequal(-1, patterns)%normal one, using patterns
                        [ismem, id] = ismember(x_end', patterns, 'rows');
                        if ismem
                            converged = true;
                            h = sprintf('Match found + found pattern p%d at iteration %d', id, iterations);
                            break;
                        elseif constant_energy>N/4
                            h = sprintf('No match found! Converged in iteration %d', iterations);
                            converged = true;
                            break;
                        else
                            converged = false;
                        ...
                        if constant_energy>N/4
                            h = sprintf('Converged in %d iterations', iterations);
                            converged = true;
                            break;
                        else
                            converged = false;
...
%%     Below is for the first task
    else %FOR FIRST TASK ONLY
        converged = false;
